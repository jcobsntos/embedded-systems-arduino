...
void loop() {
  ...
  if (currentMillis - previousMillis >= 500 && !resetState && !allOffState) {
    ...
    // Turn off the current LED before incrementing
    ...
    if (currentLedIndex >= 6) {
      ...
      updateLeds(false);
      updateHour();
      currentLedIndex = 0; // Reset the index to loop back to the first LED
    } else {
      digitalWrite(ledPins[currentLedIndex], HIGH);
    }
  }
  ...
  if (allOffState && currentMillis - previousMillis >= 500) {
    allOffState = false; 
    digitalWrite(ledPins[currentLedIndex], HIGH); // Turn on the first LED after all off
    previousMillis = currentMillis;
  }
  ...
  if (resetState && currentMillis - previousMillis >= 500) {
    resetState = false;
    digitalWrite(ledPins[currentLedIndex], HIGH); // Ensure the next LED is turned on
    previousMillis = currentMillis;
  }

  multiplexDisplay(); // Always display the current hour count
}
...
void resetHourglass() {
  ...
  if (mode == ASCENDING || mode == EVEN_ONLY) {
    hourCount = 0; 
  } else if (mode == ODD_ONLY) {
    hourCount = 1; 
  } else if (mode == DESCENDING) {
    hourCount = 23; 
  } else if (mode == ASCEND_DESCEND) {
    hourCount = 0; 
    ascendingDirection = true;  
  }
  displayNumber(hourCount);
